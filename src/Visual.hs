module Visual
    ( displayLines
    ) where

import Traversal
import Data.List(sortBy)
-- Accepts IO [FileInfo]
-- Render the desired, pretty output with necessary information
-- returns IO[String] which main can then output as it sees fit

displayLines :: [FileInfo] -> String -> [String]
displayLines infoList prefix = strList
  where trimmedList = [x | x <- infoList, (Traversal.depth x) < 3]
        -- Sorted list:
        sortedList = sortBy (\x y -> compare (show (fPath x)) (show (fPath y))) trimmedList
        strList = [(getElegantOutput x prefix) | x <- sortedList]


  -- Sort the info list lexicographically
  -- Remove all listings beyond a certain depth
  -- For each result, render an output string and append it to a list
  -- return with IO monad the string list
  --let trimmedList = [x | x <- infoList, (Traversal.depth x) < 3]
  --let strList = [show x | x <- trimmedList]

getElegantOutput :: FileInfo -> String -> String
getElegantOutput info prefix = (getPrefix info (depth info) (depth info)) ++ (rmPrefix (prefix) (fPath info)) ++ " " ++ (show $ fSize (stats info))

getPrefix :: FileInfo -> Int -> Int -> String
getPrefix info currDepth initDepth
  | currDepth == 0 = " "
  | currDepth == initDepth = (getPrefix info (currDepth - 1) initDepth) ++ "\\_"
  | otherwise = (getPrefix info (currDepth - 1) initDepth) ++ " "

-- https://stackoverflow.com/questions/18554083/haskell-program-to-remove-part-of-list-and-print-the-rest
rmPrefix :: Eq a => [a] -> [a] -> [a]
rmPrefix [] ys = ys
rmPrefix _ [] = []
rmPrefix xs ys =
  if xs == take n ys
  then drop n ys
  else ys
  where n = length xs